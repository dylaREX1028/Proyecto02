...
def run_snake_game():
    ...
    def restart_game():
        nonlocal canvas, label, snake, food, direction, score
        score = 0
        direction = 'down'
...
# Función para el próximo turno en el juego
    ...
    def create_buttons_and_labels():
        nonlocal label, start_button, instructions_button, exit_button
        label = tk.Label(window, text="Snake Game", font=('consolas', 24))
        label.pack(pady=20)
        start_button = tk.Button(window, text="Start Game", font=('consolas', 16), command=restart_game)
        start_button.pack(pady=10)
        instructions_button = tk.Button(window, text="Instructions", font=('consolas', 16), command=show_instructions)
        instructions_button.pack(pady=10)
        exit_button = tk.Button(window, text="Exit", font=('consolas', 16), command=exit_game)
        exit_button.pack(pady=10)

    # Función principal para inicializar el juego
    def initialize_game():
        nonlocal window, canvas, label, start_button, instructions_button, exit_button
...
# Si se ejecuta este archivo directamente, se inicia el juego de la serpiente
